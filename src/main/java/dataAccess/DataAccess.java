	...
	public void initializeDB(){
		...
		try {
		   ...
		   Team team32= new Team("Federer");
			...
			Quote quote4 = q2.addQuote(2.5, "2", q2);
			...
			Apustua ap6 = new Apustua(apA6, quote2);
			...
			DineroFechaMota d = new DineroFechaMota(50.0, new Date(), meterDinero);
			this.DiruaSartu(reg1, d);
			this.DiruaSartu(reg2, d);
			this.DiruaSartu(reg3, d);
			this.DiruaSartu(reg4, d);
...
public void open(boolean initializeMode){
	...
	public boolean ApustuaEgin(DatosApuesta da) {
		Registered user = (Registered) db.find(Registered.class, da.getU().getUsername());
		Boolean b;
		if(user.getDirukop()>=da.getBalioa()) {
			db.getTransaction().begin();
			ApustuAnitza apustuAnitza = new ApustuAnitza(user, da.getBalioa());
			db.persist(apustuAnitza);
			for(Quote quo: da.getQ()) {
			...
			db.getTransaction().commit();
			db.getTransaction().begin();
			int apustuBikoitzaGalarazi = da.getApustuaGalarazi();
			...
			user.updateDiruKontua(da.getBalioa());
			Transaction t = new Transaction(user, da.getBalioa(), new Date(), "ApustuaEgin"); 
			...
			for(Jarraitzailea reg:user.getJarraitzaileLista()) {
				...
				if(b) {
					DatosApuesta da2=new DatosApuesta(erab.getNork(), da.getQ(), da.getBalioa(), apustuBikoitzaGalarazi);
					if(erab.getNork().getDiruLimitea()<da.getBalioa()) {
						DatosApuesta da1= new DatosApuesta(erab.getNork(), da.getQ(), erab.getNork().getDiruLimitea(), apustuBikoitzaGalarazi);
						this.ApustuaEgin(da1);
					}else
						this.ApustuaEgin(da2);
					}
				}
			}
			return true;
